    ...
    const runQuery = (useResume: boolean) => query({
      ...
      options: {
          ...
          ? {
              mcpServers: {
                discord: {
                  ...
                  args: [
                    "--directory",
                    getConfig().DISCORD_MCP_DIR as string,
                    "run",
                    "python",
                    "-m",
                    "src.server",
                  ...
                  env: {
                    ...
                    DISCORD_GUILD_ID: getConfig().DISCORD_GUILD_ID,
                  },
                },
              },
            }
        ...
        canUseTool: async (
          ...
          // Discord MCP tools: auto-approve safe actions, require approval for
          // risky ones (delete / ban / kick / prune / permission changes).
          if (toolName.startsWith("mcp__discord__")) {
            ...
            const isRisky = riskyKeywords.some((kw) => toolName.toLowerCase().includes(kw));
            if (!isRisky) {
              return { behavior: "allow" as const, updatedInput: input };
            }
            // risky → fall through to the approval button below
          }
        ...
        try {
          for await (const message of queryInstance) {
            ...
            if ("result" in message) {
              ...
              console.log(`[session] query complete in ${resultMsg.duration_ms ?? "?"}ms, result len=${(resultMsg.result ?? "").length}`);
