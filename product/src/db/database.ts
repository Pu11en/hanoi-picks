...
// Store the DB outside the git working tree by default so a stray `git clean`
// in a parent repo can't wipe it. Override with HANOI_DB if needed.
const DB_PATH = process.env.HANOI_DB || path.join(process.cwd(), "data.db");
